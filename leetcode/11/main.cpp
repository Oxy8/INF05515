...
public:
    static int maxArea(vector<int>& height) {
        int p_inicio = 0;
        int p_fim = (height.size() - 1);


        int max_area = min(height[p_inicio], height[p_fim]);

        // Pegar sempre menor e tentar levar para lugar melhor.
        // manter dois contadores. só move 1 por vez.
        // 1 começa no inicío e o outro no fim, assim, não iremos perder candidatos.

        while (p_inicio < p_fim) {
            int curr_area = min(height[p_inicio], height[p_fim]) *(p_fim-(p_inicio));

            if ( max_area <= curr_area ) {
                max_area = curr_area;
                cout << "p_inicio " << p_inicio << " p_fim " << p_fim << " max_area " << max_area << " 11 \n"; 
            }
            
            if (height[p_inicio] < height[p_fim]) {
                p_inicio++;
            } else {
                p_fim--;
            }
...
int main() {
    ...
    for (i=0; i< test.size(); i++) {
        for (j=0; j< test.size(); j++) {
            if (maxArea <= min(test[i], test[j]) *(i-(j))) {
                maxArea = min(test[i], test[j]) *(i-(j));
                cout << "i: " << i << " j: " << j << " maxArea: " << maxArea << "\n";
            }
        }
    }
    
    int result = Solution::maxArea(test);
    cout << result;
}